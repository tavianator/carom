...
#ifndef CAROM_VECTOR_SCALAR_BINARY_PROXY_HPP
#define CAROM_VECTOR_SCALAR_BINARY_PROXY_HPP
...
namespace carom
{
  ...
  class scalar_vector_binary_proxy
  {
  public:
    scalar_vector_binary_proxy(const T& lhs, const U& rhs)
      : m_lhs(lhs), m_rhs(rhs) { }
    // scalar_vector_binary_proxy(const scalar_vector_binary_proxy& proxy);
    // ~scalar_vector_binary_proxy();
  ...
  private:
    ...
    scalar_vector_binary_proxy& operator=(const scalar_vector_binary_proxy&);
  };

  template <typename T, typename op>
  class scalar_vector_binary_proxy<T, mpfr_t, op>
  {
  public:
    scalar_vector_binary_proxy(const T& lhs,
  ...
  template <typename T, typename op>
  class scalar_vector_binary_proxy<mpfr_t, T, op>
  {
  public:
    scalar_vector_binary_proxy(mpfr_t lhs_x, mpfr_t lhs_y, mpfr_t lhs_z,
                               const T& rhs)
  ...
  class scalar_vector_binary_proxy<mpfr_t, mpfr_t, op>
  {
  public:
    scalar_vector_binary_proxy(mpfr_t lhs_x, mpfr_t lhs_y, mpfr_t lhs_z,
                               mpfr_t rhs_x, mpfr_t rhs_y, mpfr_t rhs_z)
...
#endif // CAROM_VECTOR_SCALAR_BINARY_PROXY_HPP
