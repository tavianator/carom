...
namespace carom
{
  ...
  public:
    scalar_units() { mpfr.acquire(m_fp); }
    template <typename T>
    scalar_units(const T& n) { mpfr_acquire(m_fp); mpfr_from(m_fp, n); }
    scalar_units(const scalar_units<m, d, t>& n)
    { mpfr.acquire(m_fp); mpfr_set(m_fp, n.m_fp, GMP_RNDN); }
    ~scalar_units() { mpfr.release(m_fp); }
