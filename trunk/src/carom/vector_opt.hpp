...
namespace carom
{
  ...
  public:
    vector_units() { mpfr.acquire(m_x); mpfr.acquire(m_y); mpfr.acquire(m_z); }

    vector_units(void*) { // For constructs like vector v = 0; if (v == 0) { }
      mpfr.acquire(m_x); mpfr_set_ui(m_x, 0, GMP_RNDN);
      mpfr.acquire(m_y); mpfr_set_ui(m_y, 0, GMP_RNDN);
      mpfr.acquire(m_z); mpfr_set_ui(m_z, 0, GMP_RNDN);
    }

    vector_units(const scalar_units<m, d, t>& x,
                 const scalar_units<m, d, t>& y,
                 const scalar_units<m, d, t>& z) {
      mpfr.acquire(m_x); mpfr_set(m_x, x.mpfr(), GMP_RNDN);
      mpfr.acquire(m_y); mpfr_set(m_y, y.mpfr(), GMP_RNDN);
      mpfr.acquire(m_z); mpfr_set(m_z, z.mpfr(), GMP_RNDN);
    }

    vector_units(const vector_units<m, d, t>& n) {
      mpfr.acquire(m_x); mpfr_set(m_x, n.m_x, GMP_RNDN);
      mpfr.acquire(m_y); mpfr_set(m_y, n.m_y, GMP_RNDN);
      mpfr.acquire(m_z); mpfr_set(m_z, n.m_z, GMP_RNDN);
    }
